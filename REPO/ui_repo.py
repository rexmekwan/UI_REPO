"""
Single-file Django mini-site: DemoStore E-Commerce Flow.
...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .card.dashboard-card{max-width:520px;text-align:center;}
  ...
  .service-card .icon{width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,var(--teal-600),#143f45);display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-weight:700;color:var(--mist-100);margin-bottom:16px;font-size:18px;}
  ...
  button.submit, a.submit-link{display:block;text-align:center;text-decoration:none;width:100%;padding:13px;margin-top:6px;border:none;border-radius:10px;background:var(--gold-500);color:var(--ink-900);font-family:'Sora',sans-serif;font-weight:600;font-size:14.5px;cursor:pointer;transition:transform .12s ease, box-shadow .12s ease;}
  button.submit:hover, a.submit-link:hover{transform:translateY(-1px);box-shadow:0 10px 24px -10px rgba(242,184,75,0.5);}
  button.submit:active, a.submit-link:active{transform:translateY(0);}
  button.submit:focus-visible, a.submit-link:focus-visible{outline:2px solid var(--mist-100);outline-offset:2px;}
...
<main class="card">
  <div class="mark">🛒</div>
  <h1>Welcome to DemoStore</h1>
  <p class="sub">Sign in to start shopping. Any username and password will work &mdash; this is a demo flow.</p>
...
<nav class="nav">
  <div class="nav-inner">
    <span class="brand">DemoStore</span>
    <div class="nav-links">
      <a href="/dashboard/" class="$home_cls">Home</a>
      <a href="/about/" class="$about_cls">About</a>
      <a href="/shopping/" class="$shopping_cls">Shop</a>
      <a href="/services/" class="$services_cls">Perks</a>
      <a href="/pricing/" class="$pricing_cls">Memberships</a>
...
<main class="card dashboard-card">
  ...
  <p class="sub">You're signed in to DemoStore. Browse our latest collection of premium tech and lifestyle gear.</p>
  <div style="margin-top: 24px;">
    <a href="/shopping/" class="submit-link" style="display:inline-block; width:auto; padding:12px 32px;">Browse Catalog &rarr;</a>
  </div>
</main>
""")
...
<main class="card wide">
  ...
  <p>This is a small e-commerce demo site that runs entirely from one Python file using Django. There's no database and no real accounts &mdash; the login screen accepts any username and password and starts a session.</p>
  <p>It exists to show a minimal multi-page shopping flow: a login gate, a product catalog, a working session-based cart, and a couple of supporting pages, all wired together with plain Django views and inline templates.</p>
</main>
""")
...
PRODUCTS = [
    ...
    },
    {
        ...
        'detail': 'A compact 75% mechanical keyboard with hot-swappable switches, per-key RGB lighting, and an aluminum top plate for a solid typing feel.',
    },
    {
        'id': 3, 'emoji': '⌚', 'name': 'Smart Watch', 'price': 129.50,
        'blurb': 'Heart-rate tracking, week-long battery.',
        'detail': 'Tracks heart rate, sleep, and workouts around the clock, with up to seven days of battery on a single charge and an always-on display.',
    },
    {
    ...
    {
        'id': 5, 'emoji': '🎒', 'name': 'Everyday Backpack', 'price': 75.00,
        'blurb': 'Water-resistant, 20L capacity.',
        'detail': 'A sleek, water-resistant backpack with a padded laptop sleeve, hidden security pockets, and ergonomic shoulder straps for daily commutes.',
    },
    {
        'id': 6, 'emoji': '🖱️', 'name': 'Ergonomic Mouse', 'price': 45.00,
        'blurb': 'Vertical design, Bluetooth 5.0.',
        'detail': 'A vertical ergonomic mouse designed to reduce wrist strain during long working hours. Features customizable buttons and multi-device pairing.',
    },
    {
        'id': 7, 'emoji': '🔋', 'name': 'Fast Power Bank', 'price': 29.99,
        'blurb': '10000mAh, 20W USB-C output.',
        'detail': 'A slim power bank capable of charging your phone from 0 to 50% in 30 minutes. Perfect for travel and daily emergencies.',
    },
    {
        'id': 8, 'emoji': '💻', 'name': 'Aluminum Stand', 'price': 34.50,
        'blurb': 'Adjustable, foldable design.',
        'detail': 'A sturdy aluminum stand that elevates your laptop to eye level, improving posture and device cooling. Folds flat for easy storage.',
    },
]
...
<div class="page-head">
  <h1>Shop Catalog</h1>
  <p>Click any item to view details and add it to your cart &mdash; nothing here actually ships.</p>
</div>
<div class="grid-wrap">
...
<main class="card product-detail">
  ...
  <div class="detail-links">
    <a href="/shopping/">&larr; Back to catalog</a>
...
<div class="page-head">
  <h1>Your cart</h1>
  <p>Review the items you've added before proceeding to checkout.</p>
</div>
<div class="grid-wrap">
...
<div class="grid-wrap">
  <p class="cart-empty">Your shopping cart is empty. <a href="/shopping/">Browse the catalog</a> to add something.</p>
</div>
""")

SERVICE_ITEMS = [
    ('🚚', 'Fast Shipping', 'Orders placed before 2 PM ship the same business day with tracking.'),
    ('📦', 'Easy Returns', 'Not completely satisfied? Return any item within 30 days for a full refund.'),
    ('🛡️', '1-Year Warranty', 'All our premium tech products are backed by a comprehensive 1-year warranty.'),
    ('💬', '24/7 Support', 'Our dedicated customer service team is always online to help with your orders.'),
]
...
<div class="page-head">
  <h1>Store Perks</h1>
  <p>Why millions of fake customers choose DemoStore for their tech needs.</p>
</div>
<div class="grid-wrap">
...
PRICING_TIERS = [
    ('Guest', '0', 'Standard shopping', ['Standard shipping rates', '30-day returns', 'Basic email support'], False),
    ('Plus', '9', 'For regular shoppers', ['Free 2-day shipping', '60-day returns', 'Priority support', 'Early access to sales'], True),
    ('VIP', '29', 'The ultimate experience', ['Free overnight shipping', 'Unlimited returns', '24/7 concierge', 'Exclusive VIP items'], False),
]
...
<div class="page-head">
  <h1>Store Memberships</h1>
  <p>Upgrade your shopping experience. No payment is actually processed &mdash; this is a demo.</p>
</div>
<div class="grid-wrap">
...
FAQ_ITEMS = [
    ('Is this a real store?', "No &mdash; it's a demo e-commerce site to show a small multi-page Django app running from a single file."),
    ('Does the login check my password?', 'No. Any non-empty username and password combination is accepted, by design.'),
    ('Will I actually receive the products?', "No. The cart works, but there is no checkout process and no inventory."),
    ('Where is my cart data saved?', "There's no database configured. Your cart and session data live securely in a signed browser cookie."),
]
...
<div class="page-head">
  ...
  <p>Quick answers about how this demo store works.</p>
</div>
<div class="grid-wrap">
...
<main class="card wide">
  ...
  <p>Need help with an order? Send a message below. Nothing is actually emailed &mdash; this just echoes it back to show the form works end to end.</p>
  <form method="post" novalidate>
    <input type="hidden" name="csrfmiddlewaretoken" value="$csrf">
    <div class="field">
    ...
    <div class="field">
      ...
      <textarea id="message" name="message" placeholder="How can we help?">$message</textarea>
    </div>
    $status_html
...
def shopping_view(request):
    ...
    return HttpResponse(PAGE.substitute(title='Shop', body=body))
...
def services_view(request):
    ...
    return HttpResponse(PAGE.substitute(title='Perks', body=body))
...
def pricing_view(request):
    ...
    return HttpResponse(PAGE.substitute(title='Memberships', body=body))
...
def contact_view(request):
    ...
    if request.method == 'POST':
        ...
        if not name or not email or not message:
            ...
            status_html = f'<p class="success">Thanks, {escape(name)} &mdash; our support team will reach out soon.</p>'
